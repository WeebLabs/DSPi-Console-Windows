...
public sealed partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        ViewModel.FiltersChanged += (_, _) => BodePlot.Invalidate();
    ...
    private void InitializeChannelLists()
    {
        ...
        {
            InputChannelsList.Items.Add(CreateChannelListItem(channel));
        }

        OutputChannelsList.Items.Clear();
        foreach (var channel in Channel.Outputs)
        {
            OutputChannelsList.Items.Add(CreateChannelListItem(channel));
        }
    }

    private ListViewItem CreateChannelListItem(Channel channel)
    {
        var item = new ListViewItem { Tag = channel };
    ...
    private void OnChannelSelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        if (sender == InputChannelsList && InputChannelsList.SelectedItem is ListViewItem inputItem && inputItem.Tag is Channel inputChannel)
        {
            selected = inputChannel;
            OutputChannelsList.SelectedItem = null;
        }
        else if (sender == OutputChannelsList && OutputChannelsList.SelectedItem is ListViewItem outputItem && outputItem.Tag is Channel outputChannel)
        {
            selected = outputChannel;
            InputChannelsList.SelectedItem = null;
        }

        if (selected != null)
        {
            ViewModel.UpdateChannelSelection(selected);
            ShowChannelEditor(selected);
        }
        else if (InputChannelsList.SelectedItem == null && OutputChannelsList.SelectedItem == null)
        {
